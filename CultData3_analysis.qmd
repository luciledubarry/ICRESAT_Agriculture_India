---
title: "V3- Fichier 3 : Analyse de la base Cultivation 2014"
format: html
editor: source
---
...
ggsave("figures/Top10_cultures.png", plot = plot_cultures, width = 8, height = 6, dpi = 300)
```

Je regarde comment les méanges / plots sont réparties selon les régions. 
...
Pour cela, j'ajoute la variable rank à la table distinct_hh, qui comptabilisent le nombre de ménages distincts et leur culture associé. 
...
bar_mean_hacre <- mean_hacre_crop |>
  ...
  slice_head(n = 10) |>
  ggplot(aes(x = reorder(CROP, MEAN_HACRE), y = MEAN_HACRE)) +
  geom_col(fill = "lightblue2", color = "deepskyblue4", width = 0.8) +
  coord_flip() +
  labs(
...
#### Paddy - Hr/acre moyen par opération

Ce graphique se base sur la productivité par acre (HR_PER_ACRE) et montre la moyenne de Hr/acre par opération pour le paddy. 

Tous les graphiques suivants ne considèrent seulement les champs qui ont une seule culture et le travail humain. 
...
#### Principales cultures - Hr/acre moyen par opération

##### Moyenne toutes cultures confondues
...
plot_prct_labor_time <- prct_oper |>
  ...
  labs(
    title = "Allocation of working hours by operation (in %)",
    x = "",
    y = "",
    fill = "Operation"
  ) +
...
! ajouter un filtre du nombre d'observations (min 20) !
...
### Paddy- Hr/acre selon HYV - revoir les résultats !
...
mean_hacre_oper_HYV |>
  ...
  coord_flip() +
  labs(
    title = "Paddy: Hr/acre moyen par opération selon la variété de graine",
    x = "",
    y = "",
    fill = ""
  ) +
...
```{r}
# Recalculer le ratio H/acre selon les opérations, en prennant en compte le genre
...
manque les cultures dont NA VAR_TYPE -> surtout COTTON ou 
